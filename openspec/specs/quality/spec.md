...
The project SHALL run the test suite in CI (e.g. GitHub Actions) on every push and pull request to the default branch. The workflow SHALL execute `npm run test:run` (or equivalent) and fail the build if tests fail.
